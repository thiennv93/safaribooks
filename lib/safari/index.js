 ...
 */
Safari.prototype.authorizeUser = function authorizeUser(username, password, sessionid) {
    logger.log(`sessionId=` + sessionid);
    if (sessionid) {
        logger.log(`crawl with sessionid=` + sessionid);
 ...
 */
Safari.prototype.fetchResource = function fetchResource(url, isSession, options) {
    logger.log(`fetchResource called with URL: ${url}`);
    ...
    if (isSession) {
        headers = {
            "cookie": `sessionid=${this.accessToken}`
        };
    } else {
        headers = {
 ...
 */
Safari.prototype.fetchMeta = function fetchMeta(id, isSession) {
    logger.log(`fetchMeta called with id: ${id}`);
    if (!id) return Promise.reject("id was not specified");
    // ## make meta request
    return this.fetchResource(`api/v1/book/${id}/`, isSession).then((body) => {
        if (!this.books[id]) this.books[id] = {};
 ...
 */
Safari.prototype.fetchTOC = function fetchTOC(id, isSession) {
    debug(`fetchTOC called with id: ${id}`);
    if (!id) return Promise.reject("id was not specified");
    // ## make TOC request
    return this.fetchResource(`api/v1/book/${id}/flat-toc/`, isSession).then((body) => {
        if (!this.books[id]) this.books[id] = {};
 ...
 */
Safari.prototype.fetchChapters = function fetchChapters(id, isSession) {
    ...
    let promises = this.books[id].meta.chapters.map((chapterUrl) => {
        return this.fetchResource(chapterUrl, isSession, {uri: chapterUrl}).then((chapterMeta) => {
            ...
            return this.fetchResource(chapterMeta.content, isSession, {
 ...
 */
Safari.prototype.getBookById = function getBookById(id, isSession) {
 ...
 */
Safari.prototype.fetchBookById = function fetchBookById(id, username, password, sessionid) {
