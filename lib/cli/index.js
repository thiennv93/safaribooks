...
program
  ...
  .option('-s, --sessionid <sessionid>','sessionId')
  .option('-o, --output <output>','output path the epub file should be saved to')
  .parse(process.argv);

var username, password, sessionid;
...
if ((program.sessionid)) sessionid = program.sessionid;
...
let json = JSON.stringify({ "username": username, "password": password , "sessionid": sessionid});
fs.writeFile(__dirname + '/.credentials', json).then( () => {
  debug(`the username and password and sessionid were successfully cached`);
}).catch( (err) => {
...
safariClient.fetchBookById(program.bookid, username, password, sessionid).then( (bookJSON) => {
